...
float AABB::findSplit(int &axis)
{
	float length_x = max.x - min.x;
	float length_y = max.y - min.y;
	float length_z = max.z - min.z;
	float max_axis_value = MAX3(length_x, length_y, length_z);

	if (max_axis_value == length_x)
	{
		axis = 0;
		return min.x + max_axis_value / 2;
	}
	else if (max_axis_value == length_y)
	{
		axis = 1;
		return min.y + max_axis_value / 2;
	}
	else if (max_axis_value == length_z) 
	{
		axis = 2;
		return min.z + max_axis_value / 2;
	}
	else
	{
		printf("error in findSplit");
		return -1;
	}
}
